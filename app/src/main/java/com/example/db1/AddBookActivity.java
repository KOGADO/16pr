...
public class AddBookActivity extends AppCompatActivity {
    ...
    private void addBookToDatabase(){
        ...
        if(bookName.isEmpty() || bookAuthor.isEmpty()) {
            Toast.makeText(this, "НЕ ВСЕ ПОЛЯ ЗАПОЛНЕНЫ, ЗАПОЛНИ ПЖ", Toast.LENGTH_SHORT).show();
            return;
        }
        long result =db.addBook(bookName,bookAuthor);
        if(result>0){
            Toast.makeText(this,"Телефон теперь в списке",Toast.LENGTH_SHORT).show();
            startActivity(new Intent(AddBookActivity.this,MainActivity.class));
            finish();
        }else{
            Toast.makeText(this,"Все не так, переделывай",Toast.LENGTH_SHORT).show();
        }
